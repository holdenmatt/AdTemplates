 ...
 * Carousel.js
 */
window.Carousel=(function(){if(!Function.prototype.bind){Function.prototype.bind=function(p){var s=Array.prototype.slice,o=s.call(arguments,1),n=this,r=function(){},q=function(){return n.apply(this instanceof r?this:(p||{}),o.concat(s.call(arguments)))
};r.prototype=this.prototype;q.prototype=new r();return q}}function h(n,o){for(var p in o){if(o.hasOwnProperty(p)){n.style[p]=o[p]}}}function a(p,o,r){var q=document.createElement(p);for(var n in o){if(o.hasOwnProperty(n)){q.setAttribute(n,o[n])}}h(q,r);return q}function b(p){var o=a("style",{type:"text/css"}),n=document.getElementsByTagName("head")[0];var q=document.createTextNode(p);if(o.styleSheet){o.styleSheet.cssText=q.nodeValue}else{o.appendChild(q)}n.appendChild(o)}function c(p,o){var n=new RegExp("(\\s|^)"+o+"(\\s|$)");return p.className.match(n)}function e(o,n){if(o&&!c(o,n)){o.className+=" "+n}}function k(p,o){if(p){var n=new RegExp("(\\s|^)"+o+"(\\s|$)");p.className=p.className.replace(n," ")}}function d(n){n.style.transition=n.style.webkitTransition="opacity .25s ease-out";if(n.style.webkitTransition){n.style.opacity=0}else{n.style.visibility="hidden"}}function i(o,n){var p=o.getAttribute(n);if(!p){throw new Error("Missing required attribute: "+n)}return p}function j(o,n){var q=i(o,n),p=parseInt(q,10);
if(String(p)!==q){throw new Error("Attribute must be an integer: "+n)}return p}function g(p){this.el=p;this.href=i(p,"data-href");this.width=j(p,"data-width");this.height=j(p,"data-height");this.ul=p.querySelector("ul");if(!this.ul){throw new Error("Carousel div must contain an <ul> tag")}this.items=[];var n=this.ul.querySelectorAll("li"),q,r;for(var o=0;o<n.length;o++){q=n[o];r=q.getAttribute("data-image-src");if(r){q.appendChild(a("img",{src:r}))}if(q.querySelectorAll("img").length===1){this.items.push(q)}else{q.parentNode.removeChild(q)}}if(this.items.length===0){throw new Error("Carousel ul must contain <li> items with either a child img tag or data-image-src attribute.")}}g.prototype.initialize=function(){e(this.el,"wrapper");h(this.el,{width:this.width+"px",height:this.height+"px"});for(var p=0;p<this.items.length;p++){h(this.items[p],{width:this.width+"px",height:this.height+"px"})}this.scroller=a("div",{"class":"scroller"},{width:(this.width*this.items.length)+"px",height:"100%"});
this.el.appendChild(this.scroller);this.scroller.appendChild(this.ul);this.overlay=this.el.querySelector("span.Overlay");if(this.overlay){var n=a("div",{"class":"OverlayContainer"},{width:this.width+"px"});this.scroller.appendChild(n);n.appendChild(this.overlay)}this.el.style.display="block";var o={snap:true,momentum:false,hScrollbar:false,wheelAction:"none",snapThreshold:5,onScrollStart:this.onScrollStart.bind(this),onScrollMove:this.onScrollMove.bind(this),onTouchEnd:this.onTouchEnd.bind(this)};this.iScroll=new iScroll(this.el,o);this.indicator=new m(this);this.el.appendChild(this.indicator.el)};g.prototype.onTap=function(o){var n=navigator.userAgent,p="_blank";if(n.match(/Android/i)||n.match(/Kindle/i)||n.match(/Silk-Accelerated/i)){p="_self"}window.open(this.href,p)};g.prototype.onSwipe=function(){};g.prototype.onScrollStart=function(n){if(this.overlay){d(this.overlay.parentNode);this.overlay=null}};g.prototype.onScrollMove=function(p){if(!this.scrolledOut){var o=this.iScroll.options.snapThreshold,n=this.iScroll.absDistX>o,q=this.iScroll.absDistY>o;
if(n||q){this.scrolledOut=true}}};g.prototype.onTouchEnd=function(p){if(p.type==="mouseout"){return}if(this.scrolledOut){var n=function(){p.stopPropagation();p.target.removeEventListener("click",n,false)};p.target.addEventListener("click",n,false);this.scrolledOut=false;this.onSwipe()}else{var o=this.iScroll.currPageX;this.onTap(o)}};function m(o){this.carousel=o;this.el=a("div",{"class":"PageIndicator"});var n=a("ul");this.el.appendChild(n);o.iScroll.pagesX.forEach(function(p){n.appendChild(a("li"))});this.update();this.carousel.iScroll.options.onScrollEnd=this.update.bind(this)}m.prototype.update=function(){var q=this.el.querySelector("li.active");k(q,"active");var o=this.carousel.iScroll.currPageX+1,n="li:nth-child("+o+")",p=this.el.querySelector(n);e(p,"active")};var l;var f=document.querySelectorAll("div.Carousel");if(f.length>0){l=new g(f[f.length-1])}else{throw new Error("div.Carousel not found")}document.addEventListener("DOMContentLoaded",function(){b(window.Carousel.styles);l.initialize()
