 ...
 */
window.TouchCarousel=window.TouchCarousel||{};(function(){window.TouchCarousel.open=function(g,h){window.open(g,"_blank")};window.TouchCarousel.onSwipe=function(){};var b=window.TouchCarousel.util;var f=document.querySelectorAll("div.Carousel");if(f.length>0){f=f[f.length-1]}else{console.log("Cannot activate Carousel: no ul.Carousel element found")}var c={snap:true,momentum:false,hScrollbar:false,wheelAction:"none",snapThreshold:5};function e(l,h){this.el=this.wrapper=l;this.options=h;this.width=parseInt(l.style.width,10);this.height=parseInt(l.style.height,10);if(!this.width||!this.height){throw new Error("div.Carousel must have a width/height")}this.href=l.getAttribute("data-href");if(!this.href){throw new Error("div.Carousel must have a data-href attribute")}var k=l.querySelector("ul");if(!k){throw new Error("div.Carousel must contain an <ul> tag")}var g=k.querySelectorAll("li");if(g.length===0){throw new Error("div.Carousel > ul must contain <li> items")
}this.scroller=b.createElement("div",{},{width:this.width*g.length+"px",height:"100%"});b.addClass(this.wrapper,"wrapper");b.addClass(this.scroller,"scroller");for(var j=0;j<g.length;j++){b.setStyles(g[j],{width:this.width+"px",height:this.height+"px"})}this.scroller.appendChild(k);this.wrapper.appendChild(this.scroller);this.wrapper.style.display="block";this.initializeOverlay();h.onScrollStart=this.onScrollStart.bind(this);h.onScrollMove=this.onScrollMove.bind(this);h.onTouchEnd=this.onTouchEnd.bind(this);this.iScroll=new iScroll(this.wrapper,h);this.indicator=new a(this);this.wrapper.appendChild(this.indicator.el)}e.prototype.initializeOverlay=function(){this.overlay=this.el.querySelector("span.Overlay");if(this.overlay){var g=b.createElement("div",{"class":"OverlayContainer"},{width:this.width+"px"});this.scroller.appendChild(g);g.appendChild(this.overlay)}};e.prototype.onScrollStart=function(g){if(this.overlay){b.fadeOut(this.overlay.parentNode)}};e.prototype.onScrollMove=function(i){if(!this.scrolledOut){var h=this.options.snapThreshold,g=this.iScroll.absDistX>h,j=this.iScroll.absDistY>h;
if(g||j){this.scrolledOut=true}}};e.prototype.onTouchEnd=function(i){if(i.type==="mouseout"){return}if(this.scrolledOut){var g=function(){i.stopPropagation();i.target.removeEventListener("click",g,false)};i.target.addEventListener("click",g,false);this.scrolledOut=false;window.TouchCarousel.onSwipe()}else{var h=this.iScroll.currPageX;window.TouchCarousel.open(this.href,h)}};function a(h){this.carousel=h;var g=this.el=b.createElement("ul",{"class":"PageIndicator dark"});h.iScroll.pagesX.forEach(function(i){g.appendChild(b.createElement("li"))});this.update();this.carousel.iScroll.options.onScrollEnd=this.update.bind(this)}a.prototype.update=function(){var j=this.el.querySelector("li.active");b.removeClass(j,"active");var h=this.carousel.iScroll.currPageX+1,g="li:nth-child("+h+")",i=this.el.querySelector(g);b.addClass(i,"active")};function d(){b.addStyles(window.TouchCarousel.styles);new e(f,c)}document.addEventListener("DOMContentLoaded",d,false)})();window.TouchCarousel.styles=".Carousel.wrapper{position:relative;overflow:hidden}.Carousel>.scroller>ul{margin:0;padding:0;list-style:none}.Carousel>.scroller>ul>li{position:relative;display:block;float:left;-webkit-box-sizing:border-box;box-sizing:border-box;-webkit-user-select:none;user-select:none}.PageIndicator{position:absolute;text-align:center;width:100%;height:30px;bottom:0;padding:0;margin:0}.PageIndicator>li{display:inline-block;width:8px;height:8px;-webkit-border-radius:4px;border-radius:4px;background:#ddd;margin:11px 4px 0 0}.PageIndicator>li.active{background:#888}.PageIndicator>li:last-child{margin:0}.PageIndicator.dark{background:#000;opacity:.6}.PageIndicator.dark>li{background:#555}.PageIndicator.dark>li.active{background:#fff}.Carousel .OverlayContainer{position:absolute;text-align:center;left:0;top:50%;height:40px;margin-top:-20px;z-index:100}.Carousel .Overlay{display:inline-block;padding:10px;background-color:rgba(0,0,0,0.7);color:#fff;font-weight:bold;border-radius:10px;-webkit-border-radius:10px}";