 ...
 */
window.TouchCarousel=window.TouchCarousel||{};window.TouchCarousel.util=(function(){var a={};a.setStyles=function(b,c){for(var d in c){if(c.hasOwnProperty(d)){b.style[d]=c[d]
}}};a.createElement=function(d,c,f){var e=document.createElement(d);for(var b in c){if(c.hasOwnProperty(b)){e.setAttribute(b,c[b])}}a.setStyles(e,f);return e};a.addStyleTag=function(d){var c=a.createElement("style",{type:"text/css"}),b=document.getElementsByTagName("head")[0];var e=document.createTextNode(d);if(c.styleSheet){c.styleSheet.cssText=e.nodeValue}else{c.appendChild(e)}b.appendChild(c)};a.hasClass=function(d,c){var b=new RegExp("(\\s|^)"+c+"(\\s|$)");return d.className.match(b)};a.addClass=function(c,b){if(c&&!a.hasClass(c,b)){c.className+=" "+b}};a.removeClass=function(d,c){if(d){var b=new RegExp("(\\s|^)"+c+"(\\s|$)");d.className=d.className.replace(b," ")}};a.fadeOut=function(b){b.style.display="none"};if(!Function.prototype.bind){Function.prototype.bind=function(d){var g=Array.prototype.slice,c=g.call(arguments,1),b=this,f=function(){},e=function(){return b.apply(this instanceof f?this:(d||{}),c.concat(g.call(arguments)))};f.prototype=this.prototype;e.prototype=new f();return e
}}return a})();
/*!
 * Carousel.js
 *
 ...
 */
window.TouchCarousel=window.TouchCarousel||{};(function(){var e;window.TouchCarousel.open=function(h,i){window.open(h,"_blank")};window.TouchCarousel.onSwipe=function(){};var g=document.querySelectorAll("div.Carousel");if(g.length>0){e=new f(g[g.length-1])}else{throw new Error("div.Carousel not found")}document.addEventListener("DOMContentLoaded",function(){b.addStyleTag(window.TouchCarousel.styles);e.initialize()},false);var b=window.TouchCarousel.util;function c(i,h){var j=i.getAttribute(h);if(!j){throw new Error("Missing required attribute: "+h)}return j}function d(i,h){var k=c(i,h),j=parseInt(k,10);if(String(j)!==k){throw new Error("Attribute must be an integer: "+h)}return j}function f(h){this.el=h;this.href=c(h,"data-href");this.width=d(h,"data-width");this.height=d(h,"data-height");this.ul=h.querySelector("ul");if(!this.ul){throw new Error("Carousel div must contain an <ul> tag")}this.items=this.ul.querySelectorAll("li");
if(this.items.length===0){throw new Error("Carousel ul must contain <li> items")}}f.prototype.initialize=function(){this.scroller=b.createElement("div",{"class":"scroller"},{width:(this.width*this.items.length)+"px",height:"100%"});this.el.appendChild(this.scroller);this.scroller.appendChild(this.ul);b.addClass(this.el,"wrapper");b.setStyles(this.el,{width:this.width+"px",height:this.height+"px"});for(var j=0;j<this.items.length;j++){b.setStyles(this.items[j],{width:this.width+"px",height:this.height+"px"})}this.initializeOverlay();this.el.style.display="block";var h={snap:true,momentum:false,hScrollbar:false,wheelAction:"none",onScrollStart:this.onScrollStart.bind(this),onScrollMove:this.onScrollMove.bind(this),onTouchEnd:this.onTouchEnd.bind(this)};this.iScroll=new iScroll(this.el,h);this.indicator=new a(this);this.el.appendChild(this.indicator.el)};f.prototype.initializeOverlay=function(){this.overlay=this.el.querySelector("span.Overlay");if(this.overlay){var h=b.createElement("div",{"class":"OverlayContainer"},{width:this.width+"px"});
this.scroller.appendChild(h);h.appendChild(this.overlay)}};f.prototype.onScrollStart=function(h){if(this.overlay){b.fadeOut(this.overlay.parentNode)}};f.prototype.onScrollMove=function(j){if(!this.scrolledOut){var i=this.iScroll.options.snapThreshold,h=this.iScroll.absDistX>i,k=this.iScroll.absDistY>i;if(h||k){this.scrolledOut=true}}};f.prototype.onTouchEnd=function(j){if(j.type==="mouseout"){return}if(this.scrolledOut){var h=function(){j.stopPropagation();j.target.removeEventListener("click",h,false)};j.target.addEventListener("click",h,false);this.scrolledOut=false;window.TouchCarousel.onSwipe()}else{var i=this.iScroll.currPageX;window.TouchCarousel.open(this.href,i)}};function a(i){this.carousel=i;var h=this.el=b.createElement("ul",{"class":"PageIndicator"});i.iScroll.pagesX.forEach(function(j){h.appendChild(b.createElement("li"))});this.update();this.carousel.iScroll.options.onScrollEnd=this.update.bind(this)}a.prototype.update=function(){var k=this.el.querySelector("li.active");b.removeClass(k,"active");
var i=this.carousel.iScroll.currPageX+1,h="li:nth-child("+i+")",j=this.el.querySelector(h);b.addClass(j,"active")}})();window.TouchCarousel.styles=".Carousel.wrapper{position:relative;overflow:hidden}.Carousel>.scroller>ul{margin:0;padding:0;list-style:none}.Carousel>.scroller>ul>li{position:relative;display:block;float:left;-webkit-box-sizing:border-box;box-sizing:border-box;-webkit-user-select:none;user-select:none}.PageIndicator{position:absolute;text-align:center;width:100%;height:30px;bottom:0;padding:0;margin:0;background:#000;opacity:.6}.PageIndicator>li{display:inline-block;width:8px;height:8px;-webkit-border-radius:4px;border-radius:4px;background:#555;margin:11px 4px 0 0}.PageIndicator>li.active{background:#fff}.PageIndicator>li:last-child{margin:0}.Carousel .OverlayContainer{position:absolute;text-align:center;left:0;top:50%;height:40px;margin-top:-20px;z-index:100}.Carousel .Overlay{display:inline-block;padding:10px;background-color:rgba(0,0,0,0.7);color:#fff;font-weight:bold;border-radius:10px;-webkit-border-radius:10px}";